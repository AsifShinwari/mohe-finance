<div>
    <x-backend.shared.page-nav>
          <x-slot:title>
               {{ __('leftbar.code_locations') }}
          </x-slot:title>
          <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('leftbar.home') }}</a></li>
          <li class="breadcrumb-item active">{{ __('leftbar.code_locations') }}</li>
     </x-backend.shared.page-nav>
    ...
    <x-backend.shared.page-container>
        <x-backend.shared.table>
            ...
            <x-slot:tools>
                <div>
                    {{--@hasDirectPermission('Locations - Create')
                    <a wire:navigate href="{{ route('finance.master.location.create') }}" class="btn btn-primary">
            ...
            <thead>
                ...
                <th>{{ __('master.location.prov') }}</th>
                <th>{{ __('master.location.dist') }}</th>
                <th>{{ __('master.fund.username') }}</th>
                <th class="text-center">{{ __('master.fund.actions') }}</th>
            </thead>
            <tbody>
                @foreach($this->locations as $obj)
                    <tr class="">
                        <td>{{ $loop->index+1 }}</td>
                        <td>{{ $obj->prov_code }}</td>
                        <td>{{ $obj->dist_code }}</td>
                        ...
                        <td class="d-flex justify-content-start">
                            ...
                            {{--@hasDirectPermission('Locations - Update')
