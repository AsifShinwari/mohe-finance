<div>
     <x-backend.shared.page-nav>
          <x-slot:title>
               {{ Settings::trans('Edit Program','برنامه تصحیح کړئ','تصحیح برنامه') }}
          </x-slot:title>
          <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('leftbar.home') }}</a></li>
          <li class="breadcrumb-item"><a wire:navigate href="{{ route('finance.master.program.index') }}">{{ __('leftbar.code_programs') }}</a></li>
          <li class="breadcrumb-item active">{{ Settings::trans('Edit Program','برنامه تصحیح کړئ','تصحیح برنامه') }}</li>
     </x-backend.shared.page-nav>
     <x-backend.shared.page-container>
               ...
               <div class="row m-0 p-0">
                    ...
                    <x-input 
                         name="activity_code" 
                         id="activity_code" label="{{ Settings::trans('Activity Code','د فعالیت کود','کود فعالیت') }}"
                         placeholder="{{ Settings::trans('Activity Code','د فعالیت کود','کود فعالیت') }}"
                         wire:model="activity_code" 
                         type="number" 
                         col="col-sm-3" /> 
