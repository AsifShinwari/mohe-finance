...
@section('page_title')
{{ Settings::trans('Add Budget Promises','د تعهد اضافه کول','اضافه کردن تعهد') }}
@endsection

@section('page_nav')
    <li class="breadcrumb-item active"><a href="{{ route('admin.dashboard') }}">{{ __('leftbar.home') }}</a></li>
    <li class="breadcrumb-item active"><a href="{{ route('finance.budget.plan.promises.index',$budget_plan->id) }}">{{ Settings::trans('Budget Promises','تعهدات','تعهدات') }}</a></li>
    <li class="breadcrumb-item"><a>{{ Settings::trans('Add Budget Promises','د تعهد اضافه کول','اضافه کردن تعهد') }} {{ substr(Settings::change_to_hijri($budget_plan->year),0,4) }}</a></li>
@endsection
    ...
    <form id="frm-add">
        <div class="row m-0 p-4">
            ...
            <div class="col-sm-12">
                ...
                <a href="{{ route('finance.budget.plan.promises.index',$budget_plan->id) }}" class="btn btn-secondary">
...
@push('scripts')
    <script>
        ...
        $('#code_objects_2_id').on('change', function(){
            ...
            $.ajax({
                url: "{{ route('finance.budget.plan.promises.get.code.object.3') }}",
                type: 'GET',
        ...
        $('#code_objects_3_id').on('change', function(){
            var selected = $(this).val();
            $.ajax({
                url: "{{ route('finance.budget.plan.promises.get.code.object.5') }}",
                type: 'GET',
        ...
        $('#frm-add').submit(function(e){
            ...
            $.ajax({
                url: "{{ route('finance.budget.plan.promises.store') }}",
                type: 'POST',
