...
@section('page_title')
  {{ Settings::trans('Development Budget Approved','منظور شوی انکشافي بودیجه','بودجه منظور شده انکشافی') }} {{ substr(Settings::change_to_hijri($budget_plan->year),0,4) }}
@endsection
...
@section('contents')
  <x-backend.shared.page-container :show_card="true">
        ...
        @hasDirectPermission('Approved Budget (Development) - Create')
          <a class="btn btn-primary @if($is_finalized) d-none @endif" id="add-btn" href="{{ route('finance.budget.dev.plan.create',$budget_plan->id) }}">
        ...
        @endhasDirectPermission
          ...
          href="{{ route('finance.budget.dev.plan.print',$budget_plan->id) }}">
            <i class="fa fa-print"></i>
            {{ Settings::trans('Print','پرنټ','پرنت') }}
          </a>
          
        @hasDirectPermission('Approved Budget (Development) - Lock')
    ...
    <div class="table-responsive" id="tbl-container">
      @include('backend.finance.development_budget.approved_budget.inc_tbl_body')
    </div>
...
$('#finalize').click(function(){
      if(confirm('Are you sure, to finalize this form?')) {
        $.ajax({
          url: "{{ route('finance.budget.dev.plan.finalized') }}",
          method: 'POST',
          data: {
            _token: '{{ csrf_token() }}',
