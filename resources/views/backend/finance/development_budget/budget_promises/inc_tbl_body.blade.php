  ...
  @foreach($budget_plan_promises as $item)
    <tr id="row-{{ $item->id }}">
      ...
      <td class="text-center">
          ...
          @if(!$is_finalized)
            @hasDirectPermission('B.Promises (Development) - Update')
              <a class="btn btn-sm btn-primary edit-btn" href="{{ route('finance.budget.dev.plan.promises.edit',$item->id) }}"><i class="fa fa-edit"></i></a>
            @endhasDirectPermission
            @hasDirectPermission('B.Promises (Development) - Delete')
  ...
  $(document).ready(function() {
      $('.delete-btn').click(function(){
        if(confirm('Are You Sure To Delete This Record?')){
          ...
          $.ajax({
              url: "{{ route('finance.budget.dev.plan.promises.delete') }}",
              type: 'POST',
