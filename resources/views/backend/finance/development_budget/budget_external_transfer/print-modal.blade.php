...
title="{{ Settings::trans('Budget Transfer Transaction Print','د تعدیل پرنټ','پرنت تعدیل') }}">
      <div class="modal-body pt-0">
        <div class="row">
          <form action="{{ route('finance.budget.dev.plan.transfers.external.save.form.info') }}" method="POST" id="single-print-frm">
  ...
  <script>
    function load_print_modal(id){
      loader('show');
      $.ajax({
          url: "{{ route('finance.budget.dev.plan.transfers.external.get.form.info') }}",
          type: "GET",
          data:{'id':id},
    ...
    function print_page(){
      loader('show');
      $.ajax({
          url: "{{ route('finance.budget.dev.plan.transfers.external.save.form.info') }}",
          type: "POST",
          ...
          success: function (data) {
            ...
            if(data.result==200){
              $('#print-modal').modal('show');
              window.location.replace('{{ route("finance.budget.dev.plan.transfers.external.single.form.info") }}?id='+data.id);
            }
          },
          error: function (jqXHR, textStatus, errorThrown) {
              console.log(errorThrown);
