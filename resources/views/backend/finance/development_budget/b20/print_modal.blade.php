...
<x-modal id="print-modal" title="{{ Settings::trans('Print B20','د ب-۲۰ فورم پرنټ','پرنت فورم ب-۲۰') }}">
      <div class="modal-body">
        <form action="{{ route('finance.budget.dev.plan.b20.print') }}" method="get" id="print-frm">
        <div class="row">
  ...
  <script>
    $(document).ready(function () {
        $("#add-b10-info").click(function() {
            $.ajax({
                url: "{{ route('finance.budget.dev.plan.b20.get_info') }}",
                type: "GET",
                data:{'budget_plan_id':$('#budget_plan_id').val()},
                success: function (data) {

                    $('#b20_afmis_no').val(data.dev_b20_afmis_no);
                    $('#b10_finance_drctr').val(data.dev_b3_finance_drctr);
                    $('#b10_gnrl_bgt_mgr').val(data.dev_b3_gnrl_bgt_mgr);
                    $('#b10_gnrl_drctr').val(data.dev_b3_gnrl_drctr);

                    $('#b20_beu_no').val(data.dev_b20_beu_no);
                    $('#b20_form_no').val(data.dev_b20_form_no);
                    $('#b20_needs_in_letters').val(data.dev_b20_needs_in_letters);
                    $('#b20_prescription_gnrl_drctr').val(data.dev_b20_prescription_gnrl_drctr);
                    $('#b20_sending_date').val(data.dev_b20_sending_date);
                    $('#b20_sending_no').val(data.dev_b20_sending_no);
        ...
        $("#save-b10-info").click(function() {
            $.ajax({
                url: "{{ route('finance.budget.dev.plan.b20.save-print-info') }}",
                type: "GET",
                ...
                success: function (data) {
                    ...
                    $('#b20_beu_no').val(data.dev_b3_beu_no);
                    $('#b20_form_no').val(data.dev_b20_form_no);
                    $('#b20_needs_in_letters').val(data.dev_b20_needs_in_letters);
                    $('#b20_prescription_gnrl_drctr').val(data.dev_b20_prescription_gnrl_drctr);
                    $('#b20_sending_date').val(data.dev_b20_sending_date);
