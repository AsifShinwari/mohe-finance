       ...
       <x-select 
        ...
        <x-select 
          ...
          <div class="col-sm-2 pt-4">
            ...
            @hasDirectPermission('B10 (Regular) - Create')
            ...
            <a class="btn btn-secondary" id="print-btn" data-target="#print-modal" data-toggle="modal"
              href="#" title="{{ Settings::trans('Print','پرنټ فورم ب-۱۰','پرنت فورم ب-۱۰') }}">
              <i class="fa fa-print"></i>
            </a>

            @hasDirectPermission('B10 (Regular) - Lock')
        ...
        @push('scripts')
          <script> 
            document.getElementById('search-frm').addEventListener('submit', function(event) {
              ...
              $.ajax({
                url: "{{ route('finance.budget.plan.approved.distributions.b10.filter') }}",
                method: 'GET',
                data: $(this).serialize(),
            ...
            $('#finalize').click(function(){
              if(confirm('Are you sure, to finalize this form?')) {
                $.ajax({
                  url: "{{ route('finance.budget.plan.approved.distributions.b10.finalize') }}",
                  method: 'POST',
                  data: {
                    _token: '{{ csrf_token() }}',
