...
title="{{ Settings::trans('Budget Transfer Transaction Print','د تعدیل پرنټ','پرنت تعدیل') }}">
  ...
  <script>
    function load_print_modal(id){
      loader('show');
      $.ajax({
          url: "{{ route('finance.budget.plan.transfer.external.get.form.info') }}",
          type: "GET",
          data:{'id':id},
    ...
    function print_page(){
      loader('show');
      $.ajax({
          url: "{{ route('finance.budget.plan.transfer.external.save.form.info') }}",
          type: "POST",
          ...
          success: function (data) {
            ...
            if(data.result==200){
              $('#print-modal').modal('show');
              window.location.replace('{{ route("finance.budget.plan.transfer.external.print.single.form.info") }}?id='+data.id);
            }
          },
          error: function (jqXHR, textStatus, errorThrown) {
              console.log(errorThrown);
