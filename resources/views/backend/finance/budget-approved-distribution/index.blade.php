...
@section('page_title')
  {{ Settings::trans('Form-B20','د ب-۲۰ فورم','فورم ب-۲۰') }} {{ $budget_plan->year }}
@endsection
    ...
    <div class="table-responsive">
        <table class="table table-sm table-bordered" id="main-tbl">
            <thead id="tbl-head">
              ...
              @include('backend.finance.budget-approved-distribution.table_body',[
                'b20'=>$b20  
              ])
        </table>
    </div>
  </x-backend.shared.page-container>
  @include('backend.finance.budget-approved-distribution.print_modal')
...
@push('scripts')
  <script>
    $(document).ready(function () {
      ...
      $('#create-form').submit(function(e){
        ...
        loader('show');
         ...
         $.ajax({
            url:"{{ route('finance.budget.plan.approved.distributions.store') }}",
            type:"get",
    ...
    function edit_row(id,month){
        loader('show');
         $.ajax({
            url:"{{ route('finance.budget.plan.approved.distributions.edit') }}",
            type:"get",
            ...
            success:function(res){
              ...
              $('#code_org_4').val(res.data.code_org_4).change();
    ...
    function get_b10_code_block_amount(){
      if($('#edit_id').val()!='0'){
        ...
        loader('show');
         $.ajax({
            url:"{{ route('finance.budget.plan.approved.distributions.get_b10_code_amount') }}",
            type:"get",
            data:$('#create-form').serialize(),
            success:function(res){
              if(res.result==200){
